...
function initPaging(){
	...
	$("#pagination").pagination(count, {  // 一共多少条数据
		...
		link_to : '/doit/bookAction/delPage/'+sortId+'/__id__',
		callback : function(){},
		prev_text : '上一页',
		next_text : '下一页'
	});
...
function addBindEvent(){
	...
	$("#typeSelect").bind("change", function(){
    	// 重新刷新页面
		window.location.href = "/doit/bookAction/delPage/"+$("#typeSelect").val()+"/0";
    });
...
function delBook(selectId){
	$.ajax({
		...
		success : function(data) {   
	        ...
	        // 刷新数据
            window.location.href = "/doit/bookAction/delPage/"+sortId+"/"+pageId;
	    },error :function(){   
	        alert("网络连接出错！");   
	    } 
	});
