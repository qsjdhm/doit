...
function initPaging(){
	...
	$("#pagination").pagination(count, {  // 一共多少条数据
		...
		link_to : '/doit/noteAction/delPage/'+sortId+'/__id__',
		callback : function(){},
		prev_text : '上一页',
		next_text : '下一页'
	});
...
function addBindEvent(){
	...
	$("#typeSelect").bind("change", function(){
    	// 重新刷新页面
		window.location.href = "/doit/noteAction/delPage/"+$("#typeSelect").val()+"/0";
    });
...
// 删除文章
function delArticle(selectId){
	$.ajax({
		type : "POST",
		url : "/doit/noteAction/delNote", 
		data : {
				"selectId" : selectId
		}, 
		dataType:"json",
		success : function(data) {   
	        ...
	        // 刷新数据
            window.location.href = "/doit/noteAction/delPage/"+sortId+"/"+pageId;
	    },error :function(){   
	        alert("网络连接出错！");   
	    } 
	});
