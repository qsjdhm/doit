...
function initPaging(){
	...
	$("#pagination").pagination(count, {  // 一共多少条数据
		...
		link_to : '/doit/noteAction/updatePage/'+sortId+'/__id__',
		callback : function(){},
		prev_text : '上一页',
		next_text : '下一页'
	});
...
function addBindEvent(){
	...
	$("#typeSelect").bind("change", function(){
    	// 重新刷新页面
		window.location.href = "/doit/noteAction/updatePage/"+$("#typeSelect").val()+"/0";
    });
	...
	$("#submitUpdate").bind("click",function(e){
		...
		$.ajax({
			...
			success : function(data) {   
		        ...
		        window.location.href = "/doit/noteAction/updatePage/"+$("#typeSelect").val()+"/"+pageId;
		    },error :function(){   
		        alert("网络连接出错！");   
		    } 
		});
