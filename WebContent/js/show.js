...
function validationComment(comment){
	...
	var articleID = parseInt($("#idHiddenInput").val());
	var articleTitle = encodeURI(encodeURI($("#titleHiddenInput").val()));
...
function bindZoomEvent(){
	$("#contentZoom").bind("click", function(){
		if(zoom==0){  // 应该放大
			$("#articleItems").addClass("col-md-12").removeClass("col-md-8");
			$(".newLeftDiv").css("display", "none");
			zoom++;
		}else{  // 应该缩小
			$("#articleItems").addClass("col-md-8").removeClass("col-md-12");
			$(".newLeftDiv").css("display", "block");
			zoom--;
		}
	});
}
...
function initReadNum(){
	$.ajax({
		...
		success : function(data) { 
			// 设置阅读量
			$("#articleReadNum").html(data.readNum+"人看过");
	    },error :function(){   
	        console.error("网络连接出错！");   
	    } 
	});
